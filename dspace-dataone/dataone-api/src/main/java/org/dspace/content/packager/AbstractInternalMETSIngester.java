 ...
 * @see PackageIngester
 */
public abstract class AbstractInternalMETSIngester extends AbstractPackageIngester
{
    /** log4j category */
     ...
     * same name.
     */
    protected final class MdrefManager implements METSManifest.Mdref
    {
                ...
                throws MetadataValidationException, IOException
        {
            ...
            return getFileInputStream(packageFile, params, path, null);
        }
            ...
            CrosswalkException, PackageValidationException
    {
                ...
                .hasNext();)
        {
            ...
            InputStream fileStream = getFileInputStream(pkgFile, params, path,context);
     ...
     */
    protected InputStream getFileInputStream(File pkgFile,
                                                    PackageParameters params, String path,Context context)
            throws MetadataValidationException, IOException
    {
        ...
        if (params.getBooleanProperty("manifestOnly", false))
        {
            ...
            catch (URISyntaxException so)
            {
                log
                        .error("Unable to retrieve external file from URI '"
                                + path
                                + "' for manifest-only METS package. syntax");
                return null;
            }
            catch (SQLException sql)
            {
                log
                        .error("Unable to retrieve external file from URI '"
                                + path
                                + "' for manifest-only METS package.  SQL");
                return null;
            }
            catch (AuthorizeException ae)
            {
                log.error("Unable to retrieve external file from URI '"
                        + path
                        + "' for manifest-only METS package.  Authentication error");
                return null;
            }
        }
        else
