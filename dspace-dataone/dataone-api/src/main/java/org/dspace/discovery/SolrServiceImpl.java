...
public class SolrServiceImpl implements SearchService, IndexingService {
        ...
        try {
            switch (dso.getType())
            {
                ...
                case Constants.BITSTREAM:
                    buildDocument(context, (Bitstream) dso);
                    log.info("Wrote Bitstream: " + handle + " to Index");
                    break;
                default:
                    log
    ...
    public void updateIndex(Context context, boolean force)
    {
        try {
            ...
            Bitstream[] bitstreams = Bitstream.findAll(context);
            for (Bitstream bitstream : bitstreams)
            {
                indexContent(context, bitstream, force);
    ...
    protected void buildDocument(Context context, Bitstream bitstream)
            throws SQLException, IOException {
        // Create Document
        SolrInputDocument doc = buildDocument(Constants.BITSTREAM, bitstream.getID(),
                DataOneUtil.getPid(context, bitstream), null);
        ...
        doc.addField("identifier_s", DataOneUtil.getPid(context, bitstream));
